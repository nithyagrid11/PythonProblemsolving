...
class Todolist:
    ...
    def read_all(self):
        ...
        p = self.start
        while p is not None:
            id_color = f"\033[31m {p.task_id}\033[0m"
            desc_color = f"\033[34m {p.description}\033[0m"
            if p.current_status:
                status = f"\033[32m {p.current_status}\033[0m"
            else:
                status = f"\033[33m {p.current_status}\033[0m"
            print(f"Id: {id_color} | Description: {desc_color} | Current_status: {status} | Due_date: {p.due_date.strftime('%d-%m-%Y')}")
            print()
            p = p.next
        return
    def search(self):
        ...
        p = self.start
        found = False
        print("Tasks containg the entered keyword: ")
        while p is not None:
            id_color = f"\033[31m {p.task_id}\033[0m"
            desc_color = f"\033[34m {p.description}\033[0m"
            if p.current_status:
            ...
            if keyword in p.description.lower():
                print(f"\nId: {id_color} | Description: {desc_color} | Current_status: {status} | Due_date: {p.due_date.strftime('%d-%m-%Y')}")
                print()
                found = True
            p = p.next
        if not found:
            print('No matching task is found')
    ...
    def filter(self):
        ...
        while p is not None:
            ...
            if option == '1' and p.current_status == True:
                print(f"\nId: {id_color} | Description: {desc_color} | Current_status: {status} | Due_date: {p.due_date.strftime('%d-%m-%Y')}")
                print()
            elif option == '2' and p.current_status == False:
                print(f"\nId: {id_color} | Description: {desc_color} | Current_status: {status} | Due_date: {p.due_date.strftime('%d-%m-%Y')}")
                print()
            elif option == '3' and p.due_date < today and p.current_status == False:
                print(f"\nId: {id_color} | Description: {desc_color}")
                print()
            p = p.next
    def update(self):
        if self.start is None:
        ...
        while p is not None:
            if p.task_id == enter_id:
                ...
                if p.current_status:
                    ...
                    status = f"\033[33m {p.current_status}\033[0m"
                print(f"description: {desc_color} | current_status: {status} | due_date: {p.due_date.strftime('%d-%m-%Y')}")
                print()
